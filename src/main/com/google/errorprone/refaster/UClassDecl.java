 ...
 */
@AutoValue
public abstract class UClassDecl implements UStatement, ClassTree {
  ...
  public Unifier unify(JCTree target, Unifier unifier) {
    if (target instanceof JCClassDecl) {
      // We want to skip e.g. autogenerated init members, so we do this by hand...
      Iterator<UMethodDecl> membersItr = getMembers().iterator();
      for (JCTree targetMember : ((JCClassDecl) target).getMembers()) {
        if (targetMember instanceof JCMethodDecl) {
          JCMethodDecl targetMethodDecl = (JCMethodDecl) targetMember;
